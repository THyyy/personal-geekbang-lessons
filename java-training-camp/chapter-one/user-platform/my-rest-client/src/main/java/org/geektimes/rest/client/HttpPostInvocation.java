...
class HttpPostInvocation implements Invocation {
    ...
    HttpPostInvocation(URI uri, MultivaluedMap<String, Object> headers, Entity<?> entity) {
        ...
        if (entity != null) {
            // headers 加上内容类型
            this.headers.put("Content-type", Collections.singletonList(entity.getMediaType().getType()
                    + "/" + entity.getMediaType().getSubtype()));
        }
    }
    ...
    private void setRequestEntity(HttpURLConnection connection) {
        ...
        try {
            ...
            if(this.entity.getMediaType().equals(MediaType.TEXT_PLAIN_TYPE)){
